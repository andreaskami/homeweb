...
public class MessageQueue {
	...
	// default Constructor
    public MessageQueue(String deviceName){
	...
	public void addRequestMessage(deviceLayer.Request r){
		...
		// estimate the time needed to satisfy the request
		// double transm_failure = 0.01; // percentage of transmission failures
		// double RTT            = 138;  // RTT Time in ms
		// r.predictedTime = (long) (requestQueue.size() * (RTT + Constants.DEV_REQUEST_INTERVAL * transm_failure) );
	}
	...
	public void addResponseMessage(deviceLayer.Response r,  long id){
		...
		try {
			...
			if(reQueueNewSize < 0) reQueueNewSize = 0;
		...
		try {
			...
			if(!requestQueue.isEmpty()){
				...
				long predictedTime = getNextRequestMessage().predictedTime;
				
				if(queueTime >= 0){
					MultiClientsScenario.totalWaitingTime += queueTime;
					MultiClientsScenario.counterWaitingTimes++;
				}
				
				Devices.reqQueueTimesFile.append("" + getNextRequestMessage().requestQueueSize + " " + queueTime + " " + reqRespTime); //+ " " + predictedTime);
